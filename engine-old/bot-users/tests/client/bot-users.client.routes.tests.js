(function () {
  'use strict';

  describe('Bot users Route Tests', function () {
    // Initialize global variables
    var $scope,
      BotUsersService;

    //We can start by loading the main application module
    beforeEach(module(ApplicationConfiguration.applicationModuleName));

    // The injector ignores leading and trailing underscores here (i.e. _$httpBackend_).
    // This allows us to inject a service but then attach it to a variable
    // with the same name as the service.
    beforeEach(inject(function ($rootScope, _BotUsersService_) {
      // Set a new global scope
      $scope = $rootScope.$new();
      BotUsersService = _BotUsersService_;
    }));

    describe('Route Config', function () {
      describe('Main Route', function () {
        var mainstate;
        beforeEach(inject(function ($state) {
          mainstate = $state.get('bot-users');
        }));

        it('Should have the correct URL', function () {
          expect(mainstate.url).toEqual('/bot-users');
        });

        it('Should be abstract', function () {
          expect(mainstate.abstract).toBe(true);
        });

        it('Should have template', function () {
          expect(mainstate.template).toBe('<ui-view/>');
        });
      });

      describe('View Route', function () {
        var viewstate,
          BotUsersController,
          mockBotUser;

        beforeEach(inject(function ($controller, $state, $templateCache) {
          viewstate = $state.get('bot-users.view');
          $templateCache.put('modules/bot-users/client/views/view-bot-user.client.view.html', '');

          // create mock Bot user
          mockBotUser = new BotUsersService({
            _id: '525a8422f6d0f87f0e407a33',
            name: 'Bot user Name'
          });

          //Initialize Controller
          BotUsersController = $controller('BotUsersController as vm', {
            $scope: $scope,
            botUserResolve: mockBotUser
          });
        }));

        it('Should have the correct URL', function () {
          expect(viewstate.url).toEqual('/:botUserId');
        });

        it('Should have a resolve function', function () {
          expect(typeof viewstate.resolve).toEqual('object');
          expect(typeof viewstate.resolve.botUserResolve).toEqual('function');
        });

        it('should respond to URL', inject(function ($state) {
          expect($state.href(viewstate, {
            botUserId: 1
          })).toEqual('/bot-users/1');
        }));

        it('should attach an Bot user to the controller scope', function () {
          expect($scope.vm.botUser._id).toBe(mockBotUser._id);
        });

        it('Should not be abstract', function () {
          expect(viewstate.abstract).toBe(undefined);
        });

        it('Should have templateUrl', function () {
          expect(viewstate.templateUrl).toBe('modules/bot-users/client/views/view-bot-user.client.view.html');
        });
      });

      describe('Create Route', function () {
        var createstate,
          BotUsersController,
          mockBotUser;

        beforeEach(inject(function ($controller, $state, $templateCache) {
          createstate = $state.get('bot-users.create');
          $templateCache.put('modules/bot-users/client/views/form-bot-user.client.view.html', '');

          // create mock Bot user
          mockBotUser = new BotUsersService();

          //Initialize Controller
          BotUsersController = $controller('BotUsersController as vm', {
            $scope: $scope,
            botUserResolve: mockBotUser
          });
        }));

        it('Should have the correct URL', function () {
          expect(createstate.url).toEqual('/create');
        });

        it('Should have a resolve function', function () {
          expect(typeof createstate.resolve).toEqual('object');
          expect(typeof createstate.resolve.botUserResolve).toEqual('function');
        });

        it('should respond to URL', inject(function ($state) {
          expect($state.href(createstate)).toEqual('/bot-users/create');
        }));

        it('should attach an Bot user to the controller scope', function () {
          expect($scope.vm.botUser._id).toBe(mockBotUser._id);
          expect($scope.vm.botUser._id).toBe(undefined);
        });

        it('Should not be abstract', function () {
          expect(createstate.abstract).toBe(undefined);
        });

        it('Should have templateUrl', function () {
          expect(createstate.templateUrl).toBe('modules/bot-users/client/views/form-bot-user.client.view.html');
        });
      });

      describe('Edit Route', function () {
        var editstate,
          BotUsersController,
          mockBotUser;

        beforeEach(inject(function ($controller, $state, $templateCache) {
          editstate = $state.get('bot-users.edit');
          $templateCache.put('modules/bot-users/client/views/form-bot-user.client.view.html', '');

          // create mock Bot user
          mockBotUser = new BotUsersService({
            _id: '525a8422f6d0f87f0e407a33',
            name: 'Bot user Name'
          });

          //Initialize Controller
          BotUsersController = $controller('BotUsersController as vm', {
            $scope: $scope,
            botUserResolve: mockBotUser
          });
        }));

        it('Should have the correct URL', function () {
          expect(editstate.url).toEqual('/:botUserId/edit');
        });

        it('Should have a resolve function', function () {
          expect(typeof editstate.resolve).toEqual('object');
          expect(typeof editstate.resolve.botUserResolve).toEqual('function');
        });

        it('should respond to URL', inject(function ($state) {
          expect($state.href(editstate, {
            botUserId: 1
          })).toEqual('/bot-users/1/edit');
        }));

        it('should attach an Bot user to the controller scope', function () {
          expect($scope.vm.botUser._id).toBe(mockBotUser._id);
        });

        it('Should not be abstract', function () {
          expect(editstate.abstract).toBe(undefined);
        });

        it('Should have templateUrl', function () {
          expect(editstate.templateUrl).toBe('modules/bot-users/client/views/form-botUser.client.view.html');
        });

        xit('Should go to unauthorized route', function () {

        });
      });

    });
  });
})();
