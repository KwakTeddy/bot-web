...
var address = require(path.resolve('./engine/core/action/common/address'));
var _ = require('lodash');
var globals = require(path.resolve('./engine/core/engine/common/globals'));
var async = require('async');
var concept = require(path.resolve('./engine/core/engine/concept/concept.js'));
var entity = utils.requireNoCache(path.resolve('./engine/core/engine/nlu/entity'));
var intent = utils.requireNoCache(path.resolve('./engine/core/engine/nlu/intent'));
...
function processInput(context, inRaw, callback) {
  ...
  async.waterfall([
    ...
    function(cb) {
      var dialogModule = require(path.resolve('./engine/core/action/common/dialog'));
...
function dialogTypeCheck(text, format, inDoc, context, callback) {
  // logger.debug('');
  // try {
  //   logger.debug('type.old.js:dialogTypeCheck: START ' + format.name + ' "' + text + '" inDoc: ' + JSON.stringify(inDoc));
  // } catch(e) {
  //   logger.debug('type.old.js:dialogTypeCheck: START ' + format.name + ' "' + text + '"');
  // }
