...
var utils = require(path.resolve('./engine/core/action/common/utils.js'));
var intent = require('./engine/nlu/intent');
var entity = require('./engine/nlu/entity');
var customContext = require('./dialog/custom-context.js');

var TemplateDataModule = require(path.resolve('./engine/core/template-datas.server.controller'));

function buildBot(botName, botPath, fileName, dialogs, commons)
{
    logger.systemLog('Building Bot: ' + botName + botPath);

    var build = utils.requireNoCache('./build');
...
{
    logger.systemLog('Loading Bot : ' + botName);

    // TODO 개발 시 node 재시작 안하려고 임시로
    utils.requireNoCache(path.resolve('./engine/core/action/common/dialog.js'));
    utils.requireNoCache(path.resolve('./engine/core/action/common/task'));
    utils.requireNoCache(path.resolve('./engine/core/action/common/type'));
    utils.requireNoCache(path.resolve('./engine/core/global/type/common.type'));
