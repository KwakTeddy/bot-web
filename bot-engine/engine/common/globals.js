...
{
    logger.systemLog('Load Global: [START]');
    ...
    // require(path.resolve('./bot-engine/action/common/type'));
    loadGlobals(path.resolve('./bot-engine/global'));
    loadGlobals(path.resolve('custom_modules/global'));
    logger.systemLog('Load Global: [END]');
};
...
function loadGlobals(dir)
{
    ...
    {
        logger.systemLog('loadBot: ' + dir + ' 경로 없음 ' + e);
        return;
    }
    ...
    for (var i = 0; i < files.length; i++)
    {
        ...
        if(file.endsWith('.js'))
        {
            ...
            logger.systemLog('\tloading file: ' + file);
        }
