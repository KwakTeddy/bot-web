...
var address = require(path.resolve('./bot-engine/action/common/address'));
var entity = utils.requireNoCache(path.resolve('./bot-engine/engine/nlu/entity'));
var intent = utils.requireNoCache(path.resolve('./bot-engine/engine/nlu/intent'));
...
var regexpTypeCheck = function (text, type, task, context, callback)
{
    ...
    logger.systemLog('');
    logger.systemLog('type.js:regexpTypeCheck: START ' + type.name + ' "' + text + '" inDoc: ' + JSON.stringify(task[type.name]));
    ...
    if(matched)
        logger.systemLog('type.js:regexpTypeCheck: MATCHED ' + type.name + ' "' + text + '" inDoc: ' + JSON.stringify(task[type.name]));
...
function mongoTypeCheck(text, format, inDoc, context, callback) {
  // logger.debug('');
  // try {
  //   logger.debug('type.old.js:mongoTypeCheck: START ' + format.name + ' "' + text + '" inDoc: ' + JSON.stringify(inDoc));
  // } catch(e) {
  //   logger.debug('type.old.js:mongoTypeCheck: START ' + format.name + ' "' + text + '"');
  // }
    ...
    async.waterfall([
                    ...
                    for (var i = 0; i < nlps.length; i++)
                    {
                        ...
                        try
                        {
                            ...
                            for (var j = 0; j < bot.concepts[key].length; j++)
                            {
                                ...
                                if(val.search(word) != -1 && !(format.exclude && _.includes(format.exclude, word)))
                                {
                                    if(!_.includes(matchConcepts, key))
                                        matchConcepts.push(key);
            ...
            if(matchConcepts.length > 0)
            {
                async.eachSeries(matchConcepts, function (word, _callback)
                {
                    ...
                    else
                    {
                        ...
                        if(format.query)
                            query = utils.merge(query, format.query);

                        var _query = model.find(query, format.mongo.fields, format.mongo.options);

                        if(format.mongo.sort)
                            _query.sort(format.mongo.sort);

                        if(format.mongo.limit)
                            _query.limit(format.mongo.limit || type.MAX_LIST);
        ...
        function(_cb)
        {
            ...
            if((!context.bot.dialogsetOption || context.bot.dialogsetOption.useContext !== false) && context.botUser.contexts && context.botUser.contexts.length > 0)
            {
                ...
                {
                    if(context.botUser.contexts[j].name)
                        topicKeywords.push({text: context.botUser.contexts[j].name, pos: 'Noun'});
                }
            ...
            async.eachSeries((topicKeywords.length > 0 ? topicKeywords : _nlps), function (word, _callback)
            {
                ...
                {
                    var query = {};
                    if(format.mongo.queryStatic)
                        query = format.mongo.queryStatic;
                    else
                        query = {};
                    ...
                    for(var j = 0; j < format.mongo.queryFields.length; j++)
                    {
                        try
                        {
                            if(!(format.exclude && _.includes(format.exclude, word)))
                            {
                                if(word.length == 1)
                                {
                                    query[format.mongo.queryFields[j]] = word;
                                }
                                else
                                {
                        ...
                        else
                        {
                            for(var k = 0; k < docs.length; k++)
                            {
                                    ...
                                    for(var m = 0; m < _nlps.length; m++)
                                    {
                                        ...
                                        if(Array.isArray(doc[format.mongo.queryFields[l]]))
                                        {
                                            for(var n = 0; n < doc[format.mongo.queryFields[l]].length; n++)
                                            {
                                                ...
                                                if(_matchIndex[n] != -1)
                                                {
                                                    ...
                                                    {
                                                        _matchCount[n]++; _matchCount1[n] +=3;
                                                    }
                                                    else if(_nlps[m].pos == 'Noun')
                                                    {
                                                        _matchCount[n]++;_matchCount1[n]+=2;
                                                    }
                                                    else
                                                    {
                                                        _matchCount[n]++;_matchCount1[n]++;
                                                    }

                                                    // console.log(word + ' ' + _word + ' ' + doc[format.mongo.queryFields[l]][n] + ' ' +_matchCount[n]);
                                                    if(!bMatchTotal)
                                                    {
                                                        _matchTotal[n] += doc[format.mongo.queryFields[l]][n].split(' ').length; bMatchTotal = true
                                                    };
                                                    ...
                                                    if(_matchOrgIndex[n] != -1 && (_matchMin[n] == -1 || _matchOrgIndex[n] < _matchMin[n]))
                                                        _matchMin[n] = _matchOrgIndex[n];

                                                    if(_matchOrgIndex[n] != -1 && (_matchMax[n] == -1 || _matchOrgIndex[n] + nlps[m].length> _matchMax[n]))
                                                        _matchMax[n] = _matchOrgIndex[n] + nlps[m].length;
                                                }
                                            }
                                        ...
                                        else
                                        {
                                            ...
                                            if(matchIndex != -1)
                                            {
                                                ...
                                                if(!bMatchTotal)
                                                {
                                                    matchTotal += doc[format.mongo.queryFields[l]].split(' ').length;bMatchTotal = true;
                                                }
                                                ...
                                                if(matchOrgIndex != -1 && (matchMin == -1 || matchOrgIndex < matchMin))
                                                    matchMin = matchOrgIndex;

                                                if(matchOrgIndex != -1 && (matchMax == -1 || matchOrgIndex + nlps[m].length> matchMax))
                                                    matchMax = matchOrgIndex + nlps[m].length;
                                            }
                                        }
                                    }
                                }
                                ...
                                if(!format.mongo.minMatch || matchCount >= format.mongo.minMatch)
                                {
                                    ...
                                    for(var l = 0; l < matchedDoc.length; l++)
                                    {
                                        if((Array.isArray(doc.input) && doc.input[maxMatchIndex] == matchedDoc[l].input) || (doc.input == matchedDoc[l].input))
                                        {
                                            bExist = true;
                                            break;
                                        }
                                    ...
                                    if(!bExist && ((nlps.length <= 2 && (matchCount == matchTotal || (matchCount / nlpMatchLength >= format.matchRate || matchCount1 >= format.matchCount))) || (nlps.length > 2 && (matchCount / nlpMatchLength >= format.matchRate || matchCount1 >= format.matchCount))))
                                    {
                                        if(Array.isArray(doc.input))
                                            doc.input = doc.input[maxMatchIndex];
    ...
    function(err)
    {
        ...
        if (matchedDoc.length > 0)
        {
            ...
            for (var _l = 0; _l < matchedDoc.length; _l++)
            {
                ...
                if (inDoc[format.name].length >= (format.limit || MAX_LIST))
                    break;
            }
        ...
        else
        {
            ...
            if(((context.botUser.topic && context.botUser.topic.length > 0) || (context.botUser.contexts && context.botUser.contexts.length > 0)) && (context.botUser.analytics == null || context.botUser.analytics2 == null))
            {
                ...
                {
                    context.botUser.analytics2 = true;
                }
            ...
            else
            {
                if(topicKeywords && topicKeywords.length > 0)
                    context.botUser.topic = topicKeywords;
