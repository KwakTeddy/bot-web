...
function botBuild(bot, botPath, fileName, dialogs, commons)
{
    ...
    for (var i = 0; i < files.length; i++)
    {
        ...
        if(fs.existsSync(dialogPath) && fs.statSync(file).mtime <= fs.statSync(dialogPath).mtime && dialogs == undefined)
        {
        ...
        {
            if(fs.existsSync(dialogPath) && fs.statSync(file).mtime <= fs.statSync(dialogPath).mtime)
            {
