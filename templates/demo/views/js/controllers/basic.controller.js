...
angular.module('template').controller('basicController', ['$scope', '$resource', '$cookies', function ($scope, $resource, $cookies)
{
    ...
    (function()
    {
        ChatbotTemplateService.get({ templateId: chatbot.templateId }, function(result)
        {
            ...
            ChatbotTemplateDataService.get({ botId: chatbot.id, templateName: result.name }, function(result)
            {
