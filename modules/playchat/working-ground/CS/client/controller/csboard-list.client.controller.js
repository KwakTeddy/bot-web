...
var cscontents = require('/modules/playchat/working-ground/CS/server/cs.server.model'); //db를 사용하기 위한 변수
var fs = require('fs');
...
router.get('/', function(req,res){
    ...
    cscontents.count({deleted:false},function(err, totalCount){
        // db에서 날짜 순으로 데이터들을 가져옴
        if(err) throw err;

        pageNum = Math.ceil(totalCount/limitSize);
        cscontents.find({deleted:false}).sort({date:-1}).skip(skipSize).limit(limitSize).exec(function(err, pageContents) {
            if(err) throw err;
...
router.get('/search', function(req, res){
    ...
    cscontents.count({deleted:false, $or:[{title:searchCondition},{contents:searchCondition},{writer:searchCondition}]},function(err, searchCount){
        if(err) throw err;
        pageNum = Math.ceil(searchCount/limitSize);

        cscontents.find({deleted:false, $or:[{title:searchCondition},{contents:searchCondition},{writer:searchCondition}]}).sort({date:-1}).skip(skipSize).limit(limitSize).exec(function(err, searchContents){
            if(err) throw err;
...
router.post('/', upload.array('UploadFile'),function(req, res){
    ...
    // faq 작성하고 submit하게 되면 저장이 되는 부분
    // faq 수정하고 submit하면 수정된 결과가 저장되는 부분
...
router.get('/reply', function(req, res) {
    ...
    cscontents.findOne({_id: id}, {comments: {$slice: [skipSize, limitSize]}} , function(err, pageReply){
        if(err) throw err;
...
router.get('/delete', function(req, res) {
    ...
    cscontents.update({_id:contentId}, {$set:{deleted:true}}, function(err){
        if(err) throw err;
        res.redirect('/boards');
    });
});
...
router.get('/view', function(req, res){
    ...
    cscontents.findOne({_id:contentId}, function(err, rawContent){
        if(err) throw err;
        rawContent.count += 1;
...
router.get('/password', function(req, res){
    ...
    cscontents.findOne({_id: id}, function(err, rawContents){
...
function addBoard(title, writer, content, password, upFile){
    ...
    var csContents = new cscontents;
    newcsContents.writer = writer;
    newcsContents.title = title;
    newcsContents.contents = newContent;
    newcsContents.password = password;

    newcsContents.save(function (err) {
        if (err) throw err;
        cscontents.findOne({_id: newcsContents._id}, {_id: 1}, function (err, newBoardId) {
            if (err) throw err;
            ...
            if (upFile != null) {
                ...
                for (var i = 0; i < upFile.length; i++) {
                    cscontents.update({_id: newBoardId.id}, {$push: {fileUp: renaming.fullname[i]}}, function (err) {
                        if (err) throw err;
                    });
                }
            }
...
function modBoard(id, title, content) {
    ...
    cscontents.findOne({_id:id}, function(err, originContent){
        if(err) throw err;
    ...
    cscontents.update({_id:id}, {$set: {title: title, contents: modContent, date: Date.now()}}, function(err) {
        if(err) throw err;
    });
}

function addComment(id, writer, comment) {
    cscontents.findOne({_id: id}, function(err, rawContent){
        if(err) throw err;
