...
var mongoDbTypeCheck = module.exports.mongoDbTypeCheck = function(text, format, inDoc, context, callback)
{
    logger.systemLog('');
    try
    {
        logger.systemLog('type.js:mongoDbTypeCheck: START ' + format.name + ' "' + text + '" inDoc: ' + JSON.stringify(inDoc));
    }
    catch(e)
    {
        logger.systemLog('type.js:mongoDbTypeCheck: START ' + format.name + ' "' + text + '"');
    }

    var model = mongoose.model(format.mongo.model, new mongoose.Schema(format.mongo.schema));

    var matchedDoc = [];
    var bestDoc = undefined;
    ...
    for(var i = 0 ; i < words.length; i++)
    {
        ...
        {
            _query.limit(format.mongo.limit);
        }
            ...
            else
            {
                for(var k = 0; k < docs.length; k++)
                {
                    ...
                    for(var l = 0; l < format.mongo.queryFields.length; l++)
                    {
                        ...
                        {
                            if(words[m].search(REG_ESCAPE) != -1)
                            {
                                continue;
                            }
                            ...
                            if(matchIndex != -1)
                            {
                                ...
                                if(matchOrgIndex != -1 && (matchMax == -1 || matchOrgIndex + words[m].length> matchMax))
                                {
                                    matchMax = matchOrgIndex + words[m].length;
                                }
                            }
                        }
                    }
            ...
            if(wordsCount >= words.length)
            {
                ...
                if(bestDoc)
                {
                    ...
                    for (var l = 0; l < format.mongo.queryFields.length; l++)
                    {
                        ...
                        if (matchText == '')
                        {
                            matchText = matchText.concat(_text);
                        }
                        else
                        {
                            matchText = matchText.concat(' ', _text);
                    ...
                    {
                        if (Array.isArray(inDoc['_' + format.name]))
                        {
                            inDoc['_' + format.name].push(matchOriginal);
                        }
                        else
                        {
                            inDoc['_' + format.name] = [inDoc['_' + format.name], matchOriginal];
                        }
                    }
                    else
                    {
                    ...
                    {
                        if (Array.isArray(inDoc[format.name]))
                        {
                            inDoc[format.name].push(matchText);
                        }
                        else
                        {
                            inDoc[format.name] = [inDoc[format.name], matchText];
                        }
                    }
                    else
                    {
                ...
                else if(matchedDoc.length > 0)
                {
                    ...
                    for(var _l = 0; _l < matchedDoc.length; _l++)
                    {
                        ...
                        if(inDoc.typeDoc.length >= format.limit)
                        {
                            break;
                        }
                    }
