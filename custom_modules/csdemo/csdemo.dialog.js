...
var dialogs = [
{
  ...
  task:   {action: csdemo.repairableChecktwo},
  output: [
  ...
  {if: 'context.dialog.address.시도명 && context.dialog.address.법정읍면동명 == undefined && context.dialog.address.도로명 == undefined && context.dialog.address.건물본번 == undefined', output: '고객님, 정확한 안내를 드리기 위하여 +address.시도명+ +address.시군구명+의 하위 상세주소를 입력 부탁드립니다.', 
    children: [
    {
      id: 'csdemo10',
      filename: 'csdemo',
      ...
      output: [
      {if: 'context.dialog.address.시도명 && context.dialog.address.법정읍면동명 == undefined && context.dialog.address.도로명 == undefined && context.dialog.address.건물본번 == undefined', output: {repeat: 1, options: {output: '고객님, 정확한 안내를 드리기 위하여 +address.시도명+ +address.시군구명+의 하위 상세주소를 입력 부탁드립니다.'}}}, 
      ...
      {if: 'Array.isArray({regexp: first}context.dialog.address)', output: '다음 중 어떤 지역이신가요?\n#address#+index+. +지번주소+ +시군구용건물명+\n#|다음 중 어떤 지역이신가요?', 
        children: [
        {
          id: 'csdemo8',
          filename: 'csdemo',
...
},
{
  ...
  filename: 'csdemo',
  name: '주요지명컨셉검색',
  input: {types: [{name: 'dong', typeCheck: csdemo.dongTypeCheck, raw: true}]},
...
},
{
  ...
  filename: 'csdemo',
  name: '근처검색',
  input: [/([^\b]*)(?:\b|\s).*(근처|주변).*(센터)/, /([^\b]*)(?:\b|\s)*(에서).*(센터)/],
  task:   {action: csdemo.searchNaver},
  output: [
  {if: '!(context.dialog.item)', output: '죄송합니다. 지금 입력하신 주소는 찾을수 없는 주소입니다.\n 아래 예시와 같이 다시 말씀해주세요.\n1.동명: 가산동, 구로동\n2.지번주소: 금천구 가산동 60-3\n3.도로명주소: 금천구 디지털로9길 68\n4.건물명: 여의도 트윈타워\n5.지하철역: 여의도역|죄송합니다. 지금 입력하신 주소는 찾을수 없는 주소입니다.아래 예시와 같이 다시 말씀해주세요.'}, 
  {if: 'context.dialog.item', output: {call:'서비스센터정보'}}]
},
{
  id: 'csdemo42',
...
},
{
  ...
  {output: '+center.svc_center_name+의 위치정보입니다\n 주소: +center.address3+\n 영업시간 : 평일: +center.winter_week_open+ ~ +center.winter_week_close+, 토요일: +center.winter_sat_open+ ~ +center.winter_sat_close+\n 전화번호: +center.phone+\n +center.parking+\nhttp:\/\/map.daum.net\/link\/map\/+center.svc_center_name+,+center.lat+,+center.lng+ \n해당 서비스 센터의 방문 예약을 하시겠습니까?|위치정보는 다음과 같습니다. 방문예약을 하시겠습니까?', 
    children: [
        {
          id: 'csdemo92',
          filename: 'csdemo',
...
},
{
  ...
  input: '주소',
  output: [
  {if: csdemo.locationNotExists, output: {returnCall: '위치찾기', options: {returnDialog: '주소안내', output: '고객님 근처의 서비스 센터를 파악한 후에 주차가능여부를 안내해드려도 될까요?'}}}, 
  {output: '+center.address3+입니다. 더 필요하신 게 있으신가요?', 
    children: [
    {
      id: 'csdemo213',
      filename: 'csdemo',
