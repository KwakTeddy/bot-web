...
var globalDialogs = [
  ...
  { input: {if: orderTypes.orderDialogCondition, regexp: /^9$/}, output: {up : 1}},
  { input: {if: orderTypes.orderDialogCondition, regexp: /^0$/}, output: {call: '주문취소'}},
  { input: {if: orderTypes.orderDialogCondition, regexp: /^<$/}, output: {call: '이전페이지'}},              // TODO 이전페이지, 다음페이지 구현
  { input: {if: orderTypes.orderDialogCondition, regexp: /^>$/}, output: {call: '다음페이지'}},
        ...
        output: [
          ...
          { if: function(dialog, context, callback) {
              ...
              restaurantType.typeCheck(context.dialog.inRaw, restaurantType, dialog.task, context, function(_inRaw, inDoc, matched) {
                if(matched && context.dialog.restaurant && !Array.isArray(dialog.task.restaurant)) {
      ...
      { name: '메뉴선택1', input: false,
        output: [
          ...
          { if: function(dialog, context, callback) {
              ...
              // franchiseMenuType.typeCheck(context.dialog.inRaw, franchiseMenuType, dialog.task, context, function(_inRaw, inDoc, matched) {
              //   if(matched && dialog.task.menu && !Array.isArray(dialog.task.menu)) {
              //     context.dialog.menu = dialog.task.menu;
              //     callback(true);
              //   } else callback(false);
          ...
          { if: 'context.dialog.menu == undefined',
            ...
            children: [
              { input: {types: [orderTasks.franchiseMenuType], if: 'context.dialog.menu && !Array.isArray(context.dialog.menu)'},
                output: {call: '메뉴추가확인1'}
              },
