  ...
  actions: [
    // 홈서비스 메뉴보기
    {
      ...
      xpath: {
        ...
        doc: {
          ...
          id: '//div[@class="cart_left"]/div[1]/input/@value'
        }
      },
    ...
    // 상품담기
    {
      ...
      preCallback: function(task, context, callback) {
        task.param.pId = task.preTask.doc[0].id;
        callback(task, context);
      },
      postCallback: function(task, context, callback) {
        callback(task, context);
    ...
    // 주문하기 클릭, 로그인 안한 경우 로그인 화면 redirect
    {
      ...
      action: 'xpath',
      paramDefs: [
        {type: 'String', name: 'smsAuth', display: '인증번호', isReuired: true, question: '인증번호를 말씀해 주세요.'}
      ],
      url: 'https://homeservice.lotteria.com',
    ...
    // sms 인증 요청
    {
      ...
      method: 'POST',
      url: 'https://homeservice.lotteria.com',
      path: '/RIA/api/json/member/PhoneAuth.json.asp',
      headers: {
        'Content-type' : 'application/x-www-form-urlencoded; charset=UTF-8',
      ...
      preCallback: function(task, context, callback) {
        // task.param.phone1 = task.phone1;
        // task.param.phone2 = task.phone2;
        // task.param.phone3 = task.phone3;
